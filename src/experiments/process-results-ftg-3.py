    ...
    with open(csvfilename, 'a') as csvfile:
        for subdirectory in os.listdir(directory):
            ...
            if os.path.isdir(sd) and subdirectory.startswith('A-'):
                ...
                print(di.alg)
                for i in range(len(di.instance_number)):
                    for j in range(len(di.conds[i])):
                        print(di.alg, di.benchmark, di.constant, di.lambda_, di.numruns, di.instance_number[i], j + 1, f'{di.conds[i][j]:.3f}', di.retries[i][j], '\"' + di.termination_reasons[i] + '\"', sep=',', file=csvfile)
...
with open(csvfilename, 'w') as csvfile:
    print('alg,benchmark,constant,lambda_,numruns,instance,basis_size,cond,retries,term_reason',file=csvfile)
