...
def probabilistic_subtree_mutation(tree: object, mutation_rate: float, max_depth: int = 6):
    if myrandom.RND.random() < mutation_rate:
...
def uniform_subtree_mutation(tree: object, max_depth: int = 6):
    mutation_point = myrandom.RND.randint(0, tree.size() - 1)
    subtree = ParseTree()
...
def subtree_crossover(ptree1, ptree2, crossover_rate):
    if myrandom.RND.random() < crossover_rate:
        if ptree1.size() <= 1 or ptree2.size() <= 1:
            return ptree1, ptree2

        crossover_point1 = myrandom.RND.randint(1, ptree1.size() - 1)
        crossover_point2 = myrandom.RND.randint(1, ptree2.size() - 1)
